...
from si.src.neural_networks.optimizers import Optimizer
...
class Dropout(Layer):
    '''
    Dropout layer of a neural network. Drops certain neurons to avoid overfitting.
    '''

    def __init__(self, probability: float):
        '''
        here the dropout layer will be initialized.

        parameters:
        probability -> The probability of dropping a neuron between 0 and 1 (dropout rate).
    ...
    def forward_propagation(self, input: np.ndarray, training: bool) -> np.ndarray:
        '''
        this function performs forward propagation on the given input.

        parameters:
        input -> The input to the layer.
        training -> Whether the layer is in training mode or in inference mode.
    ...
    def backward_propagation(self, output_error: np.ndarray) -> float:
        '''
        this function performs backward propagation on the given output error.
        it returns input_error=dE/dX to feed the previous layer.

        parameters:
        output_error -> The output error of the layer.

        returns:
        float -> The input error of the layer.
        '''
    ...
    def output_shape(self) -> tuple:
        '''
        this function returns the shape of the output of the layer.
        '''
        return self.input_shape()
    
    def parameters(self) -> int:
        '''
        this function returns the number of parameters of the layer.
        '''
        return 0
