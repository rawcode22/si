...
class Dataset:
    ...
    def dropna(self) -> None:
        """
        Drops all samples null value/s in them
        """

        nvsamples = np.isnan(self.X).any(axis=1)

        self.X = self.X[~nvsamples]

        if self.y is not None:
            self.y = self.y[~nvsamples]

    
    def fillna(self, value: Union[float, str]) -> None:
        """
        Replaces any null values with a chosen value.
        If a value is not specified, it will replace it with the mean or the median of the correspondent feature.
        """
